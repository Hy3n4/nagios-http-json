...
def parseArgs():
	...
	parser.add_argument('-Z', '--zabbix-check', dest='zabbix', help='Use as Zabbix item check.', action='store_true')
...
if __name__ == "__main__":
	...
	if args.zabbix:
		try:
			...
			if args.auth:
				cred = (args.auth).split(":")
				base64str = base64.encodestring(args.auth).replace('\n', '')
				req.add_header('Authorization', 'Basic %s' % base64str)
				req.add_header('Content-Type', 'application/json')
				login_data = '{"jsonrpc": "2.0", "method": "user.login", "params": { "user": "' + cred[0] + '", "password": "' + cred[1] + '" }, "id": 1}'
			if args.timeout and args.data:
				response = urllib2.urlopen(req, timeout=args.timeout, data=login_data, context=ctx)
			elif args.timeout:
				response = urllib2.urlopen(req, timeout=args.timeout, context=ctx)
			elif args.data:
				response = urllib2.urlopen(req, data=login_data, context=ctx)
			else:
				response = urllib2.urlopen(req, data=login_data, context=ctx)
			...
			if 'error' in logindata:
				nagios.append_critical("Error[%s], url:%s" % (logindata['error']['data'], url))
				print nagios.getMessage()
				exit(nagios.getCode())
			else:
				authid = logindata['result']
	...
	try:
		...
		if args.timeout and args.data:
			...
			response = urllib2.urlopen(req, timeout=args.timeout, context=ctx)
		elif args.data and args.zabbix:
			datatoparse = args.data
			lastcurlypos = datatoparse.rfind("}")
			authstr = ',"auth": "' + authid + '","id": 1'
			datahash = list(datatoparse)
			datahash.insert(lastcurlypos, authstr)
			newdata = ''.join(datahash)
			response = urllib2.urlopen(req, data=newdata, context=ctx)
		elif args.data:
	...
	if args.zabbix:
		try:
			...
			req.add_header('Authorization', 'Basic %s' % base64str)
			req.add_header('Content-Type', 'application/json')
			response = urllib2.urlopen(req,'{"jsonrpc": "2.0", "method": "user.logout", "auth": "' + authid + '", "id": 1}')
		except HTTPError as e:
			...
			jsondata = response.read()
			logoutdata = json.loads(jsondata)
		if (logoutdata == 'False'):
			nagios.append_unknown("LogoutError, url:%s" % url)
